...
function CarMapplace({ editDriverId, navigation }) {
  ...
  const { addDriver, updateDriver ,fetchDriverData } = useContext(DriverContext);
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            let userData = null;
            if (editDriverId) {
                userData = await fetchDriverData(editDriverId);
            }
            if (userData && Object.keys(userData).length > 0) {
                setProfileData({
                    ...profileData,
                    ...userData,
                    profileImage: userData.profileImage || '/path/to/default/image.jpg'
                });
                setIsEditing(true);
            } else {
                setIsEditing(false);
    ...
    fetchData();
}, [editDriverId, fetchDriverData]);  // Include fetchDriverData if it’s a prop or context function
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
         updateDriver(editDriverId, dataToUpdate);
      } else {
        addDriver(dataToUpdate);
      }
      ...
      setLoading(false);  // Ensure loading state is reset whether success or fail
    }
  };
